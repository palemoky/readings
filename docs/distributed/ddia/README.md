...
![](imgs/system-load-descriptions.png){ align=right width=50% }
不同的系统有不同的负载描述方式：如网站为每秒的请求量 rps、聊天室为同时在线人数、数据库为读写比、缓存为命中率。

### 图状数据模型

![图状数据模型示例](imgs/graph.png){ align=right width=50% }
...
![ 数仓和简化的 ETL 过程](imgs/data-warehouse-etl.png){ align=right width=50% }
...
- 场景：
    - 多数据中心。在一个数据中心内使用多主节点意义不大，其复杂性已超过所带来的好处。对于多数据中心，在每个数据中心内，采用常规的主从复制方案；而在数据中心间，由各个数据中心的主节点来负责同其它数据中心的主节点进行数据的交换、更新
        ![横跨多个数据中心的多主节点复制模型](imgs/multi-datacenter-multi-leader-replication.png){ align=right width=50% }
...
<figure markdown>
  ![节点无响应](imgs/network-fault-indistinguishable.png){ width=60% }
...
<div class="grid cards" markdown>
- <figure>
    ![分布式锁的不正确实现](imgs/distributed-lock-incorrect-implementation.png)
    <figcaption>分布式锁的不正确实现：客户端 1 的锁租约其实已经过期，但它自认为有效，最终导致文件破坏</figcaption>
  </figure>
- <figure>
    ![递增 fencing 令牌](imgs/fencing-token-implementation.png)
