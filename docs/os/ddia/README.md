...
![不同的系统有不同的负载描述方式：如网站为每秒的请求量 rps、聊天室为同时在线人数、数据库为读写比、缓存为命中率。](imgs/system-load-descriptions.png)
...
![分布式锁的不正确实现：客户端 1 的锁租约其实已经过期，但它自认为有效，最终导致文件破坏](imgs/distributed-lock-incorrect-implementation.png)

分布式锁的不正确实现：客户端 1 的锁租约其实已经过期，但它自认为有效，最终导致文件破坏

![递增 fencing 令牌，拒绝旧令牌的写操作以确保存储安全](imgs/fencing-token-implementation.png)
...
在这个过程中，参与者做出了可以提交的承诺，协调者做出了提交事务的承诺，二者的承诺都是不可撤销的，这两个承诺保证了 2PC 的原子性。

![参与者在投赞成票之后 ，协调者发生崩溃，则数据库 1 不清楚是否应该提交或中止](imgs/two-phase-commit-coordinator-crash.png)
