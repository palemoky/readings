...
![磁盘结构：A-磁道（最外圈是 0 号）；B-几何扇区；C-磁道扇区（第一个扇区是引导扇区）；D-簇（扇区组）。](imgs/disk-structure.png)

磁盘结构：A-磁道（最外圈是 0 号）；B-几何扇区；C-磁道扇区（第一个扇区是引导扇区）；D-簇（扇区组）。
...
| 2（利用海明码在 raid0 基础上增加纠错能力）  |          |          |          |          |          |                                |                        |                    | 相比 raid0 而言，数据安全性有所提升             | 读写都需要校验码，导致性能较低，使用较少 |
| 4（单独一块盘用于存储校验码）               | 3        | 1        | n-1      |          |          |                                |                        |                    |                                                 |                                          |
| 5（每块盘都有其他盘的校验码）               | 3        | 1        | n-1      | n-1      | n-1      | 中下至中                       | 追求最大容量、最小预算 | 个人、小型企业备份 | 解决 raid4 单一校验码盘的性能问题，**使用最多** | 同时两块盘损坏，则数据无法恢复           |
...
文件描述符（File Descriptor）只是一个整数，是每个进程私有的，在 Unix 系统中用于访问文件。[@I/O 多路复用](https://www.notion.so/Linux-Unix-88e4c8dbb93645d1a08b91170f59ac77?pvs=21) [@系统级 I/O](https://www.notion.so/92f1cd452dcf41b483993f28c1f2872d?pvs=21)

`strace`可以跟踪程序在运行时所做的每个系统调用。[@Linux 性能优化](https://www.notion.so/Linux-1a88534b95214b5ba9fd98b95473b1df?pvs=21)
...
2. 不能链接到目录（避免循环链接）
3. 不能链接到其他磁盘分区中的文件（因为 inode 号在特定文件系统中唯一，而不是跨文件系统（注：ext4、NTFS、FAT32 是不同的文件系统）。） | 1. 在目标目录中创建新的文件名
4. inode 的引用计数+1 |
   | 软连接（或符号链接） | 1. 有独自的 inode 号
5. 软连接本身是一种文件类型
6. 源文件被删则软连接文件也失效
7. 软链接目标文件的大小主要取决于源文件名的长短 | 1. 在目标目录中创建新的文件名
8. 创建新的 inode，新的 inode 中包含指向原始文件的路径 |
...
![耗尽 inode 导致系统异常的示例 1](imgs/9f1c2c967ad014102cd632fbefeaa503.jpeg)

![耗尽 inode 导致系统异常的示例 2](imgs/e42d46761734d92c810519440d3d45c7.png)
...
![多次访问/foo/bar 文件，路径中的每一级都要读取其 inode 信息，并更新文件的最后访问时间](imgs/path-resolution-inode-updates.png)

多次访问/foo/bar 文件，路径中的每一级都要读取其 inode 信息，并更新文件的最后访问时间
...
![创建文件的 I/O 过程](imgs/file-creation-io-steps.png)
...
![现代 DBMS 数据写入顺序](imgs/modern-dbms-data-write.png)
