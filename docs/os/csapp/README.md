...
- **Linux 对 I/O 设备的抽象是文件，网络的抽象则是套接字**（socket，socket 在英文中是插座的意思，网络套接字可以理解为当前计算机连接上因特网后，字节流就可以在网络上流动，就像电器插上插座，电流就会在电线中流动一样）。
  ![Unix I/O、标准 I/O 和 RIO](imgs/io-functions.png)
...
- 当客户端发起一个连接请求时，客户端套接字地址中的端口是由内核自动分配的临时端口。
  ![因特网应用程序的硬件和软件组织](imgs/internet-app-structure.png){ align=right width=50% }
...
- **小端序（Little Endian）**：“小的一端在前”。也就是把**低位字节**放在较小的起始内存地址处。它在内存里的排布为 `0D 0C 0B 0A`。现在市面上绝大多数个人电脑（包括基于 Intel x86 架构的 CPU）默认都采用小端序。
    ...
    ![大端序与小端序](imgs/endianness.png){ align=right width=50% }
