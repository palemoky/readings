...
![Memory Hierarchy](imgs/memory-hierarchy.png)
...
![Process Abstraction](imgs/process-abstraction.png)
...
![根据数据类型来说，在大多数编程语言中，数组和对象通常被动态分配在堆上，因为它们的大小不固定，并且需要在运行时进行动态分配和释放内存；字符串的存储位置通常取决于编程语言和字符串的类型。在某些编程语言中，字符串常常是不可变的，因此它们可能存储在常量区或堆上。但在其他情况下，编译器可能会将字符串存储在栈上，特别是对于局部变量的字符串或者短期字符串；基本数据类型（如整数、浮点数）通常会被存储在栈上，因为它们的大小固定。](imgs/virtual-memory-layout.png)
...
![System Abstractions](imgs/system-abstractions.png)
...
![Disk Surface Structure](imgs/disk-surface-structure.png)

![Disk Geometry](imgs/disk-geometry.png)

![From [PC1512](https://en.wikipedia.org/wiki/PC1512)。在DOS系统时代，电脑前左右两个盘孔分别是A盘（主驱动器）、B盘（辅助驱动器），分别用于加载操作系统和存储数据应用程序，因此在之后的Windows磁盘分区中，是以C开始的。](imgs/pc1512-floppy-drives.png)
...
![Disk Read Write Head](imgs/disk-read-write-head.png)
...
![Exceptional Control Flow](imgs/exceptional-control-flow.png)
...
![File Descriptor Table](imgs/file-descriptor-table.png)

![File Sharing 1](imgs/file-sharing-1.png)

![File Sharing 2](imgs/file-sharing-2.png)

![File Sharing 3](imgs/file-sharing-3.png)
...
![Concurrent Threads](imgs/concurrent-threads.png)
