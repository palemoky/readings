...
![协议层级与端口信息](imgs/protocol-layer-model.webp)

<div class="grid cards" markdown>
-  <figure>
    ![协议层级对比](imgs/protocol-layer-comparison.webp)
    <figcaption>协议层级对比</figcaption>
  </figure>
-  <figure>
    ![协议封装与分用](imgs/protocol-encapsulation.webp)
...
<div class="grid cards" markdown>
-  <figure>
    ![IPv4 头部字段](imgs/ipv4-header-format.webp)
    <figcaption>IPv4 头部字段(头部大小可变)</figcaption>
  </figure>
-  <figure>
    ![IPv6 头部字段](imgs/ipv6-header-format.webp)
...
!!! info
    ...
    `127.0.0.1` 和 `::1` 可以用 [localhost](http://localhost) 来代替。
...
<figure markdown>
  ![大端序与小端序](imgs/big-endian-little-endian.webp){ width="80%" }
...
<figure markdown>
  ![NAT vs NAPT](imgs/nat-vs-napt.webp)
...
> 该部分内容推荐查看 [Linux 防火墙 - 鸟哥](../../os/linux-server-setup/README.md#第-7-章-防火墙设置)
...
<figure markdown>
  ![iptables Table & Chain](imgs/iptables-table-to-chain.webp){ width="50%" }
...
![TCP 握手过程](imgs/tcp-handshake-process.webp){ align=left width="50%" }
...
!!! note "`TIME_WAIT` 与 `CLOSE_WAIT`"

    | 状态       | 发生端            | 产生的原因                                                           |  解决方法                                                               |
    | ---------- | ------------------------- |  ------------------------------------------------------------------- | ---------------------------------------------------------------------- |
    | `TIME_WAIT`  | 主动关闭方 (通常是客户端) | 配置或架构问题。连接数过多、端口耗尽。比如客户端没有 `keep-alive` 建立大量连接，并使这些连接几乎在同一时间关闭。 |  使用长连接 (Keep-Alive)；调整系统参数 (如允许重用端口 `tcp_tw_reuse`)。  |
    | `CLOSE_WAIT` | 被动关闭方 (通常是服务端) | 纯代码 Bug / 业务阻塞问题，收到对方关闭请求后，自己的代码没有调用 `close()`。                      | 排查服务器代码，确保无论发生什么异常，最终都执行 socket 层的 `close()`。 |

    这个 [线上大量 CLOSE_WAIT 分析](https://dayutalk.cn/2018/12/08/%E7%BA%BF%E4%B8%8A%E5%A4%A7%E9%87%8FCLOSE_WAIT%E5%88%86%E6%9E%90/) 的案例中，故障流程是：Server 向 MySQL 发起操作请求 -> Server 代码缺少 `commit`/`rollback` 导致 MySQL 超时发起关闭请求 -> Server 处于 `CLOSE_WAIT` 状态。在这个故障中，Server 是请求发起方，但最后却有大量 `CLOSE_WAIT` 状态，就需要使用 `ss`(`netstat`) 先查看 TCP 连接状态，再用 `tcpdump` 抓包分析，最后用 `perf` 快速定位到代码。
...
<div class="grid cards" markdown>
-  <figure>
    ![TCP 半关闭（少见）](imgs/tcp-half-close.webp)
    <figcaption>TCP 半关闭（少见）</figcaption>
  </figure>
-  <figure>
    ![TCP 同时连接与关闭（少见）](imgs/tcp-simultaneous-open-close.webp)
    <figcaption>TCP 同时连接与关闭（少见）</figcaption>
  </figure>
-  <figure>
    ![TCP 客户端与服务端通信交互过程](imgs/tcp-client-server-interaction.webp)
    <figcaption>TCP 客户端与服务端通信交互过程</figcaption>
  </figure>
-  <figure>
    ![TCP 状态转换图](imgs/tcp-state-transition.webp)
...
<div class="grid cards" markdown>
- <figure>
    ![发送端窗口](imgs/tcp-sender-window.webp)
    <figcaption>发送端窗口</figcaption>
  </figure>
- <figure>
    ![接收端窗口](imgs/tcp-receiver-window.webp)
...
<div class="grid cards" markdown>
- <figure>
    ![慢启动和拥塞避免](imgs/tcp-slow-start-congestion-avoidance.webp)
    <figcaption>慢启动和拥塞避免（Reno）</figcaption>
  </figure>
- <figure>
    ![快重传与快恢复](imgs/tcp-fast-retransmit-fast-recovery.webp)
    <figcaption>快重传与快恢复（New Reno）</figcaption>
  </figure>
</div>
...
<div class="grid cards" markdown>
- <figure>
    ![TCP Reno 算法](imgs/tcp-reno-algorithm.webp)
    <figcaption>Reno 算法（cwnd 表现为锯齿）</figcaption>
  </figure>
- <figure>
    ![TCP CUBIC 算法](imgs/tcp-cubic-algorithm.webp)
    <figcaption>CUBIC 算法（https://www.slideshare.net/deawooKim/cubic-kdw）</figcaption>
  </figure>
</div>
...
![CUBIC](imgs/tcp-cubic-workflow.webp){ align=left width=45% }
...
- **BBR 的破局（基于模型）**：BBR 抛弃了丢包信号，改为主动交替探测网络的**最大瓶颈带宽（BtlBw）**和**最小往返时间（RTprop）**。它致力于将发送速率精准控制在“刚好填满物理管道，但绝对不让多余的数据包在路由器中排队”的完美数学临界点。
- **核心优势**：
    1. **无视非拥塞丢包**：在跨国网络、弱 WiFi 等由于物理抖动导致随机丢包的环境中，CUBIC 会误以为拥塞而把速度降至龟速，而 BBR 则由于模型测算的带宽不变，依然能无视丢包跑满全速。
    2. **超低延迟**：彻底干掉了排队导致的 Bufferbloat 顽疾，使得网络在满载甚至超载吞吐下，依然保持着丝滑的低延迟。
...
<figure markdown>
  ![多次重复 ACK 触发快速重传](imgs/tcp-fast-retransmit-duplicate-ack.webp)
...
- <figure markdown>
    ![TCP 可靠传输解决方案](imgs/tcp-reliable-transmission.webp)
...
![DHCP 交换](imgs/dhcp-exchange-process.webp){ align=left width=50% }
...
![WebSocket。](imgs/websocket-upgrade-http.webp){ align=right width=50% }
...
1. 防火墙、代理、网关等中间件通常只支持 HTTP 协议，不支持 WebSocket
2. 可以在握手阶段复用 HTTP 的能力，如身份验证、TLS 加密等

ws（WebSocket）和 wss（WebSocket Secure）类似于 http 和 https。

## 安全

> 该部分内容推荐查看 [《图解密码技术》](../../crypto/cryptography/README.md)
...
<figure markdown>
  ![TLS 1.2 协议结构](imgs/tls-1.2-protocol-structure.webp){ width=60% }
...
<div class="grid cards" markdown>
- <figure>
    ![HTTPS 工作图解](imgs/https-working.webp)
    <figcaption>HTTPS 工作图解</figcaption>
  </figure>
- <figure>
    ![TLS 握手协议](imgs/tls-handshake-protocol.webp)
...
<div class="grid cards" markdown>
  ...
  - <figure>
    ![服务端返回经私钥签名的公钥，客户端验证证书链](imgs/tls-certificate-key-exchange.webp)
    <figcaption>服务端返回经私钥签名的公钥，客户端验证证书链</figcaption>
  </figure>
  - <figure>
    ![服务器验证客户端身份](imgs/tls-server-verify-client.webp)
...
- **SYN Flood（半连接泛洪攻击）**：在 TCP 三次握手阶段，攻击者发送大量 SYN 包却故意不回传 ACK 包，导致服务器的半连接队列耗尽，最终无法受理正常用户的新连接请求。传统的防御机制依赖于开启 `SYN Cookies` 防御，而现代架构（如 Linux 6+ 内核）已支持依靠 eBPF 技术在网卡层面极速过滤并丢弃恶意包。
- **全连接攻击**：攻击者按规矩完成完整的三次握手，与服务器建立合法的长连接。随后，攻击者刻意不发送业务数据，或者以极慢的速率发送请求（例如 Slowloris 滴水攻击），并持续保持连接活跃。这会导致服务器的全连接队列及线程资源被渐渐耗尽。通常需要借助代理层（如 Nginx）的请求超时限制、限制单 IP 并发连接数，或接入高防 WAF 等方式来防护。
...
- **流量注入**：部分运营商在介入网络响应时，通过恶意篡改明文传输的网页代码，强行插入 `<script>` 或 `<iframe>` 标签以额外展示广告内容。可通过全站升级至 HTTPS 进行强制加密防范。
