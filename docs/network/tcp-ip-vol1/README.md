...
> 该部分内容推荐查看 [Linux 防火墙 - 鸟哥](../../os/linux-server-setup/README.md#第7章 防火墙设置)
...
???+ note "`TIME_WAIT` 与 `CLOSE_WAIT`"
    ...
    这个 [线上大量CLOSE_WAIT分析](https://dayutalk.cn/2018/12/08/%E7%BA%BF%E4%B8%8A%E5%A4%A7%E9%87%8FCLOSE_WAIT%E5%88%86%E6%9E%90/) 的案例中，故障流程是：Server 向 MySQL 发起操作请求 -> Server 代码缺少 `commit`/`rollback` 导致 MySQL 超时发起关闭请求 -> Server 处于 `CLOSE_WAIT` 状态。在这个故障中，Server 是请求发起方，但最后却有大量 `CLOSE_WAIT` 状态，就需要使用 `ss`(`netstat`) 先查看 TCP 连接状态，再用 `tcpdump` 抓包分析，最后用 `perf` 快速定位到代码。
...
- **TCP RST 攻击**：攻击者伪造源 IP（伪装成服务端或客户端），并通过网络嗅探获取当前 TCP 会话的序列号（Sequence Number），然后向通信的某一方发送一个带有 RST 标志的包，接收方会认为对方发生了严重错误，瞬间强制关闭连接（连四次挥手都不走），直接销毁套接字。该攻击方式可用于 HTTP 和 HTTPS 连接，因为 HTTPS 只是对传输的数据加密，TCP 头部需对所有中间设备（如路由器）可见才能完成可靠传输，无法被加密。该攻击方式常见于拦截 Google 等未经审查的网站，如 2010 年时 Google 搜索“胡萝卜”或导致 Google 服务因连接被重置而短暂不可用。解决方式则是使用基于 UDP 的 QUIC 协议，由于 UDP 没有 RST 标志，也就无法被 RST 攻击。
- **SYN Flood（半连接泛洪攻击）**：在 TCP 三次握手过程中，攻击者发送大量 SYN 包，但不发送 ACK 包，导致服务器的半连接队列耗尽，无法接受新的连接。Linux 6+ 可通过 eBPF 缓解此问题。
- **全连接攻击**：攻击者完成完整的三次握手，与服务器建立真实的长连接。但在连接建立后，攻击者什么数据也不发，或者发送得极慢（类似 Slowloris 攻击的底层原理），然后保持连接不断开，以耗尽服务器的全连接队列。可通过超时限制、限制单 IP 并发连接数、WAF 等方式缓解。
- **零窗口攻击**：攻击者和服务器建立连接后，在发送的 ACK 确认包里，故意把自己的 TCP 窗口大小（Window Size）设置为 0。服务器看到客户端窗口为 0，就会停止发送数据，并把数据保留在内核的发送缓冲区中。同时，服务器为了确认客户端什么时候有空间，会不断开启定时器发送“窗口探测包（Zero Window Probe）”。这不仅耗尽了服务器的内存（堆积数据），还浪费了 CPU，导致服务器崩溃。正常的网络抖动或接收方磁盘缓冲区写满，可在毫秒内恢复，而攻击者会持续发送零窗口，可通过零窗口限时断开连接可缓解。

### DNS

- **DNS 污染**：用户在查询某个域名时，攻击者会利用物理距离的时延优势，抢先响应一个虚假的 DNS，导致无法正常访问。解决办法是使用 DoH 或 DoT，将 DNS 请求加密，让攻击者无法知道想要访问的域名。
- **DNS 劫持**：某些运营商（ISP）会劫持其拦截的 DNS 请求，将用户跳转到其导航页或宣传页面。

### TLS

- **TLS 降级攻击**：攻击者在握手过程中，通过发送特定的消息，诱导客户端和服务器降低加密套件的安全性，例如从 AES-256 降级到 AES-128，或者从 SHA-256 降级到 SHA-1。这使得攻击者更容易破解加密数据或伪造身份。
- **SNI 劫持**：由于在建立加密通道前需要在 SNI 中包含域名信息，攻击者从而获知用户访问的域名，进而将数据包丢弃或重置连接。解决方式则是使用依赖 DoH/DoT 的 ECH（Encrypted Client Hello），将 SNI 信息在 TLS 握手前加密。
- **重放攻击**：攻击者虽然无法解密 HTTPS 的报文，但他不需要看懂，只需要把拦截到的别人这段合法且已加密的 HTTP 请求包（比如：转账 100 元的操作），抓下来后原样重复发送上百次给服务器。如果上层协议设计中没有防重放机制（比如 Token/Nonce 随机数或严格的 Timestamp 校验），就会被重复执行。

### HTTP

- **流量注入**：部分运营商在响应网页代码中插入一段 `<script>` 或 `<iframe>` 代码，用于展示其广告内容。可通过升级到 HTTPS 解决。
