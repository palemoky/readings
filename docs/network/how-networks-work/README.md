...
??? question "出现大量 `TIME_WAIT` 和 `CLOSE_WAIT` 时怎么处理？"
    ...
    `CLOSE_WAIT` 则是被动关闭方的状态，产生的原因是未能正常接收到 ACK 包，因此需要排查主动关闭方代码，查看是否正常关闭连接。[这里](https://juejin.cn/post/6844903734300901390)有一个真实 `CLOSE_WAIT` 故障的排查案例，在此案例中，造成本机大量 `CLOSE_WAIT` 的原因在于，服务端代码只开启了事务，而没有 `commit`/`rollback`，导致 MySQL 在连接超时后主动发起关闭，服务端进入 `CLOSE_WAIT` 后由于缺少 `commit`/`rollback` 而无法关闭连接，最终导致连接资源耗尽，响应出现 504。
