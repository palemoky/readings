...
???+ question "为什么不能用两次握手？[视频讲解](https://youtu.be/Iuvjwrm_O5g?t=143)"
    ...
    **场景二**：假设服务端响应 SYN+ACK 包后即认为连接建立完成，则服务端开始发送数据，但此时可能网络出现故障，导致客户端掉线，服务端则无法与客户端完成正常通信。
    ...
    三次握手的另一个场景是视频或语音会议时，A 说"可以听到吗？"，B 回答"可以听到"，A 说"那我们开始吧"。
...
9. 客户端经过链路层、网络层、传输层解包。将数据写入缓冲区，缓冲区满通知应用程序读取
    ...
    - OSPF 用于自治域（AS）中的路由选择，BGP 则用于 AS 间的路由选择。
...
-  <figure>
    ![AS 内 BGP 路由表](imgs/as-bgp-routing.png)
    <figcaption>AS 内 BGP 路由表</figcaption>
  </figure>
-  <figure>
