  ...
  <table>
    <tr>
      <td align="center" style="vertical-align: bottom;">
        <img src="imgs/tcp-3-way-handshake-4-way-wave.png" alt="TCP 三次握手与四次挥手" />
      </td>
      <td align="center" style="vertical-align: bottom;">
        <img src="imgs/tcp-retransmission.png" alt="TCP 重传机制" width="60%"/>
      </td>
    </tr>
  </table>
</div>
...
???+ question "为什么不能用两次握手？[视频讲解](https://youtu.be/Iuvjwrm_O5g?t=143)"
    ...
    两次握手时，我们分别从客户端发送 SYN 包丢失和服务端响应的 SYN+ACK 包丢失两种场景说明。
...
??? question "为什么客户端需要等待超时时间？[视频讲解](https://youtu.be/Iuvjwrm_O5g?t=340)"

    如果客户端发送完 ACK 包后直接释放连接，但这个 ACK 包在网络中丢失，则会导致服务端停留在最终的确认关闭状态。如果客户端停留一个等待超时时间，服务端未能收到最终的 ACK 包时，会重新向客户端发送 FIN 包，这样就能保证连接的优雅关闭了。

??? question "出现大量 `TIME_WAIT` 和 `CLOSE_WAIT` 时怎么处理？"

    根据 TCP 的四次挥手可知，`TIME_WAIT` 是主动关闭连接一方所产生的状态，因此大量的 `TIME_WAIT` 等待其 2MSL（Maximum Segment Lifetime 报文最大生存时间，它是任何报文在网络上存在的最长时间，超过这个时间报文将被丢弃）后自动超时关闭即可，产生的原因则是自身短时间内产生了大量的请求，控制请求的产生即可。
...
3. TLS 握手
    ...
    - 客户端根据本机存储的可信 CA 证书列表和 OCSP 校验证书是否有效，校验成功后用证书中的非对称加密的公钥加密客户端生成的伪随机数发送给服务器，
    - 服务器通过私钥解密其中的随机数，作为通信的对称加密 AES 密钥。
    - 客户端使用对称加密密钥加密`Finished` 给服务器，
    - 服务器使用第一次收到的对称密钥解密`Finished`消息，如果一致，则在接下来的 TLS 会话中使用这个对称密钥。
4. 封装数据包
...
![LAN口左侧的绿灯常亮表示线路正常](imgs/lan-port-led-indicators.jpeg){ align=right width=30% }
...
- 内部路由选择协议 IGP
    ...
    - 中间系统到中间系统 IS-IS。该协议是 ISP 骨干网上最常用的 IGP 协议
          ...
          <tr>
          <td align="center" style="vertical-align: bottom;">
          <img src="imgs/rip-protocol-hop-count.png" alt="RIP 协议图示" />
          <p class="img-caption">RIP 协议</p>
          </td>
          <td align="center" style="vertical-align: bottom;">
          <img src="imgs/ospf-protocol-link-state.png" alt="OSPF 协议图示" />
          <p class="img-caption">From <a href="https://www.youtube.com/watch?v=wmR2HwkrSdY">OSPF 协议</a></p>
          </td>
          </tr>
          </table>
          </div>
...
<div align="center">
  <table>
    <tr>
      <td align="center" style="vertical-align: bottom;">
        <img src="imgs/as-bgp-routing.png" alt="AS 内 BGP 路由表" />
        <p class="img-caption">AS 内 BGP 路由表</p>
      </td>
      <td align="center" style="vertical-align: bottom;">
        <img src="imgs/tcp-reliable-transmission-solutions.png" alt="TCP 可靠传输解决方案" width="50%" />
        <p class="img-caption">TCP 可靠传输解决方案</p>
      </td>
    </tr>
  </table>
</div>
