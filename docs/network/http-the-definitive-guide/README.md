...
<figure markdown>
  ![HTTP 与 HTTPS 协议栈](imgs/http-https-protocol-stack.webp){ width="80%" }
</figure>
...
- 端对端首部（End-to-end）：分在此类别中的首部会转发给请求 / 响应对应的最终接收目标，且必须保存在有缓存生成的响应中，另外规定它必须被转发
    ...
    - Upgrade：用于检测 HTTP 协议及其他协议是否可使用更高的版本进行通信，其参数值可以用来指定一个完全不同的通信协议。比如升级到 HTTPS、WebSocket、HTTP/2、HTTP/3 等。
...
-  <figure>
    ![TCP 数据包结构](imgs/tcp-packet-struct.webp)
</figure>
-  <figure>
    ![串行 HTTP 时间线](imgs/http-transaction-delay.webp)
</figure>
</div>
...
<figure markdown>
  ![串行、管道、持久连接对比](imgs/persistent-connection.webp){ width=70% }
</figure>
...
服务器的工作流程：
![Web 服务器工作流](imgs/web-server-workflow.webp){ align=right width=50% }
...
通常基于以下原因使用代理：
![Proxy Server](imgs/proxy-server.webp){ align=right width=70% }
...
<figure markdown>
  ![盲中继 Keep-Alive 挂起](imgs/blind-relay-keepalive.webp){ width=80% }
</figure>
...
![爬虫根集](imgs/crawler-root-set.webp){ align=right width=50% }
...
<figure markdown>
  ![URL 别名与机器人环路](imgs/url-alias-robot-loop.webp){ width=80% }
...
当客户端收到 401 时，填写账号密码后，会在`Authorization`首部附加经 base64 编码的`账号:密码`形式的认证信息。如以下请求的认证信息即为`username:password`的编码结果。

<figure markdown>
  ![基本认证流程](imgs/basic-auth-flow.webp){ width=60% }
</figure>
...
<figure markdown>
  ![HTTP 认证首部](imgs/http-auth-headers.webp){ width=80% }
</figure>
...
非对称加密是使用一对密钥进行加解密，分别称为公钥和私钥，使用公钥加密的内容只能用私钥解密，使用私钥加密的内容只能用公钥解密。常见的非对称加密算法有 RSA、DSA、ECDSA，其中使用最广泛的算法是 RSA。
...
- 可以被正确地打包（Content-Length 和 Content-Encoding）
- 是最新的（通过实体验证码和缓存过期控制）
