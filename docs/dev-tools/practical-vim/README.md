...
| `` `{mark} ``                               | 跳转到指定标记                                                   |
| ``` `` ``` / `` `. `` / `` `^ `` / `` `0 `` | 跳转到上次跳转前 / 修改 / 插入的位置 / 退出 Vim 时的位置           |
| `` `[ `` / `` `] ``                         | 上次修改或复制的起始 / 结束位置                                  |
| `` `< `` / `` `> ``                         | 上次高亮选区的起始 / 结束位置                                    |
| `` y`a ``                                   | 复制当前位置到标记 a 的内容                                      |
...
语法：`[count] + Operator + Motion/TextObject`，操作符连按两次作用于当前行（如 `dd`、`yy`）。操作符详见[文本操作](#文本操作)，Motion 详见[词跳转](#%E8%AF%8D%E8%B7%B3%E8%BD%AC)与[页面内搜索](#%E9%A1%B5%E9%9D%A2%E5%86%85%E6%90%9C%E7%B4%A2)，TextObject 详见[文本对象](#%E6%96%87%E6%9C%AC%E5%AF%B9%E8%B1%A1)。
...
| `g<Ctrl+a>`         | 创建递增序列                                                |
| `<` / `>`           | 左 / 右缩进（详见[缩进](#%E7%BC%A9%E8%BF%9B)）              |
| `p` / `P`           | 在光标**后** / **前**粘贴，光标停在粘贴内容的**起始**位置   |
| `gp` / `gP`         | 同 `p` / `P`，但光标停在粘贴内容的**末尾**位置              |
| `]p`                | 粘贴并自动调整缩进至当前行层级                              |
...
| 指令        | 说明                                                    |
| ----------- | ------------------------------------------------------- |
| `>>` / `<<` | 将当前行向右 / 左缩进，宽度由 `shiftwidth` 控制         |
| `>%` / `<%` | 向右 / 左缩进 `()` 或 `{}` 内的区域（光标需置于括号上） |
| `>ib`       | 向右缩进 `()` 内的区域                                  |
| `>at`       | 向右缩进 `<>` 标签内的区域                              |
| `==`        | 自动缩进当前行                                          |
| `{n}==`     | 自动缩进 n 行（如 `3==`）                               |
...
| 指令        | 说明                                      |
| ----------- | ----------------------------------------- |
| `zf`        | 定义折叠（需配合 motion / 选区使用）      |
| `zd`        | 删除光标位置的折叠                        |
| `za`        | 切换光标位置折叠的展开 / 关闭状态（一次） |
| `zo` / `zc` | 展开/关闭光标位置的折叠（一次）           |
| `zO` / `zC` | 递归展开/关闭光标处的所有折叠             |
| `zr` / `zm` | 降低/提高一级折叠层级（展开/关闭一级）    |
| `zR` / `zM` | 递归展开/关闭所有折叠                     |
| `zi`        | 开启 / 关闭折叠功能（`foldenable` 开关）  |
| `[z` / `]z` | 跳到当前折叠的开始 / 结束位置             |
...
| 指令                     | 说明                                  |
| ------------------------ | ------------------------------------- |
| `:bp[rev]` / `:bn[ext]`  | 上 / 下一个 buffer                    |
| `:bf[irst]` / `:bl[ast]` | 第一个 / 最后一个 buffer              |
| `:b[uffer] {n}`          | 跳转到编号 n 的 buffer                |
| `:b[uffer] {name}`       | 根据名称跳转                          |
| `:bd[elete]` / `:5,10bd` | 删除 buffer / 删除编号 5~10 的 buffer |
| `:ls` / `:buffers`       | 列出所有打开的 buffer                 |
| `:cd %`                  | 设置当前 buffer 的工作目录            |
...
Vim 在编辑文件时会自动创建一个 `.swap` 文件（如 `.foo.txt.swp`），用于崩溃恢复。

**触发场景与处理**

| 场景                        | 原因                                     | 处理方式                                    |
| --------------------------- | ---------------------------------------- | ------------------------------------------- |
| 打开文件时提示 swap 已存在  | 上次编辑未正常退出，或同一文件被多处打开 | `R` 恢复未保存内容；`D` 删除 swap；`Q` 放弃 |
| 同一文件被两个 Vim 实例打开 | 出现"Already opened by another program"  | 找到另一个 Vim 进程关闭，或只读打开 `:view` |
| 误删文件后想恢复            | swap 文件仍保留最后一次编辑状态          | `vim -r foo.txt` 或在提示中按 `R` 恢复      |

**常用命令**

| 指令                    | 说明                                         |
| ----------------------- | -------------------------------------------- |
| `vim -r`                | 列出所有可恢复的 swap 文件                   |
| `vim -r {file}`         | 从 swap 文件恢复指定文件                     |
| `:recover`              | 在 Vim 内部执行恢复（等同于 `vim -r`）       |
| `:set swapfile?`        | 查看当前 swap 是否启用                       |
| `:set noswapfile`       | 临时关闭 swap（不推荐，崩溃无法恢复）        |
| `:set directory={path}` | 指定 swap 文件存放目录（避免工作目录被污染） |

> **最佳实践**：恢复完成后，手动删除对应的 `.swp` 文件，或在 Vim 内执行 `:!rm %:p:r.swp`。
