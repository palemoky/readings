...
<figure markdown>
  ![Kafka 架构体系](imgs/kafka-architecture.webp){ width=80% }
  <figcaption>Kafka 架构体系</figcaption>
</figure>

如图所示，Kafka 由多个 **Producer**、**Broker**、**Consumer** 和一个 **ZooKeeper/KRaft** 组成。ZooKeeper/KRaft 负责集群元数据的管理、选举操作等。Producer 将消息发送到 Broker，Broker 负责将收到的消息存储到磁盘中，而 Consumer 负责从 Broker 订阅并消费消息。
...
Broker 2                             // 多个分区分散在不同 Broker 上，方便水平扩容提高吞吐量
  |--Topic-Partition 1 (Follower)    // 同步状态分为 ISR（In-Sync Replicas）和 OSR（Out-of-Sync Replicas），通常只有 ISR 副本有候选资格
  |--Topic-Partition 2 (Leader)      // 分区内通过 offset 确保消息的顺序性，HW（High Watermark）与 LEO（Log End Offset）分别标识消费进度和写入位置

Broker 3                             // 同一 Broker 对同一分区只能存放一个副本（replication factor=3 则需至少 3 个 Broker）
  |--Topic-Partition 1 (Follower)
  |--Topic-Partition 2 (Follower)
```

消费者组（Consumer Group）是 Kafka 中用于实现**高可用和负载均衡**的核心机制。
```
Group 1:                           // Partition 的水平扩容是为了增加吞吐量，消费者组中的多个消费者则是为了增加消费能力，为了避免重复消费，一个 Partition 在同一 Group 内只能被一个 Consumer 消费，但一个 Consumer 可以消费多个 Partition（单向约束）。
  Consumer A -> Topic Partition 1  // 为了平衡生产与消费能力，建议组内的consumer与partition数量一致
  Consumer B -> Topic Partition 2
  Consumer C -> Topic Partition 3

Group 2:
  Consumer D -> Topic Partition 1
  Consumer E -> Topic Partition 2
  Consumer F -> Topic Partition 3
```
...
- 主题（Topic）：消息以主题为单位进行分类，生产者将消息发送到特定的主题，消费者订阅主题并进行消费。
...
- <figure>
    ![分区偏移量说明](imgs/partition-offsets.webp)
    <figcaption>分区偏移量说明</figcaption>
  </figure>
- <figure>
    ![](imgs/hw-leo-sync.webp)
...
- 生产者需要用序列化器（Serializer）把对象转换成字节数组才能通过网络发送给 Kafka。消费者需要用反序列化器（Deserializer）把从 Kafka 中收到的字节数组转换成相应的对象
...
<figure markdown>
  ![生产者客户端的整体架构](imgs/producer-architecture.webp){ width=80% }
...
消息收集器中为每个分区都维护了一个**双端队列**，队列中存储了 ProducerBatch（消息批次），ProducerBatch 中包含多个 ProducerRecord，这样使字节的使用更加紧凑。**消息写入缓存时，追加到双端队列的尾部；Sender 读取消息时，从双端队列的头部读取**。
...
- `linger.ms`：指定生产者发送 ProducerBatch 之前等待更多消息（ProducerRecord）加入 ProducerBatch 的时间，默认值为 0。增大这个参数的值会增加消息的延迟，但是同时能提升一定的吞吐量。该参数与 TCP 协议中的 Nagle 算法有异曲同工之妙。
...
![消费者与消费组](imgs/consumer-groups.webp){ width=30% align=right }
...
![消费位移](imgs/consumer-offsets.webp){ width=30% align=left }

Kafka 中的消费是基于拉模式的。消费位移存储在 Kafka 内部的主题`__consumer_offsets`中，消费者在消费完消息之后需要执行消费位移的提交。
...
在 RabbitMQ 中，死信一般通过 broker 端存入，而在 Kafka 中原本并无死信的概念，只需要使用一个单独的topic作为死信队列即可。

### 延时队列

!!! Note "时间轮"

    ![时间轮结构](imgs/time-wheel-structure.webp){ width=40% align=right }
...
RabbitMQ 和 RocketMQ 支持延时队列，但 Kafka 并不支持延时队列，因此可以将延迟消息持久化存储，然后通过一个时间轮来批量检查到期任务，然后投递到Kafka中消费。
...
优先队列不同于先进先出队列，优先级高的消息具备优先被消费的特权，从而为下游提供不同消息级别的保证。优先队列只有在 Broker 存在消息堆积时才有意义，若消息被即时消费，则优先级无从体现。

Kafka 本身不支持优先队列，可通过以下几种方式实现：

1. **多 Topic 分级**（最常用）：按优先级创建不同的 Topic（如 `topic-high`、`topic-low`），消费者优先拉取高优先级 Topic。优点是实现简单、优先级隔离清晰；缺点是 Topic 数量随优先级层级增多而膨胀，且需要消费端自行协调拉取策略。
2. **同 Topic 多 Partition 分级**：同一 Topic 内用不同 Partition 表示不同优先级，生产端根据优先级选择目标 Partition。与方式一本质类似，但避免了 Topic 膨胀。
3. **消息内嵌优先级 + 消费端重排序**：生产端在消息 Header 中标记优先级，投递到同一 Topic/Partition；消费端拉取后在内存中按优先级重排序再处理。优点是对 Kafka 的 Topic/Partition 结构无侵入；缺点是消息入队后无法修改优先级，且需要缓冲消息，增加延迟和内存开销。
...
有序分为全局有序和局部有序，通常都是只局部有序，比如不同订单间的消息可以乱序，但同一订单的消息必须有序。

Kafka 仅保证**单个 Partition 内**的消息顺序，跨 Partition 无顺序保证。因此，需要严格有序的消息应通过相同的 Key 路由到同一 Partition。

即使在单 Partition 内，消费端处理失败时也需注意：失败消息只能**在本地重试**，不能投入重试队列（Dead Letter Queue），否则后续消息会先于重试消息被消费，导致乱序。因此顺序消息的消费端必须做好**幂等性**设计，以应对重试带来的重复消费。

由于以上约束，端到端的严格顺序性在分布式场景下难以完全保证，工程实践中通常采用**数据对账与补偿机制**来兜底，确保最终一致性。
...
1. **日志删除（Log Retention/Delete）**：直接删除旧数据。可以通过时间配置（如 `log.retention.hours=168` 保留 7 天）或大小配置（如 `log.retention.bytes`）来触发删除机制。
