...
链接: https://book.douban.com/subject/26912767/
来源: 纸质书
封面: https://img1.doubanio.com/view/subject/l/public/s29195878.jpg
ISBN: 9787111544937
...
![Process Abstraction](images/process-abstraction.png)
...
![根据数据类型来说，在大多数编程语言中，数组和对象通常被动态分配在堆上，因为它们的大小不固定，并且需要在运行时进行动态分配和释放内存；字符串的存储位置通常取决于编程语言和字符串的类型。在某些编程语言中，字符串常常是不可变的，因此它们可能存储在常量区或堆上。但在其他情况下，编译器可能会将字符串存储在栈上，特别是对于局部变量的字符串或者短期字符串；基本数据类型（如整数、浮点数）通常会被存储在栈上，因为它们的大小固定。](images/virtual-memory-layout.png)
...
![System Abstractions](images/system-abstractions.png)
...
磁盘是由一个或多个叠放在一起的盘片组成的，它们被封装在一个密封的包装里，整个装置则被称为磁盘。

**柱面**是指所有盘片表面上到主轴中心的距离相等的磁道的集合。

![Disk Surface Structure](images/disk-surface-structure.png)

![Disk Geometry](images/disk-geometry.png)

![From [PC1512](https://en.wikipedia.org/wiki/PC1512)。在DOS系统时代，电脑前左右两个盘孔分别是A盘（主驱动器）、B盘（辅助驱动器），分别用于加载操作系统和存储数据应用程序，因此在之后的Windows磁盘分区中，是以C开始的。](images/pc1512-floppy-drives.png)

From [PC1512](https://en.wikipedia.org/wiki/PC1512)。在DOS系统时代，电脑前左右两个盘孔分别是A盘（主驱动器）、B盘（辅助驱动器），分别用于加载操作系统和存储数据应用程序，因此在之后的Windows磁盘分区中，是以C开始的。
...
![Disk Read Write Head](images/disk-read-write-head.png)

![多盘片示意图](images/multi-platter-disk.png)
...
![Exceptional Control Flow](images/exceptional-control-flow.png)
...
![Interrupt Exception](images/interrupt-exception.png)

![Trap Exception](images/trap-exception.png)

![Fault Exception](images/fault-exception.png)

![Abort Exception](images/abort-exception.png)
...
- Linux 信号
  ...
  | 21    | IGTTIN    | 停止直到下一个 SIGCONT | 后台进程从终端读               |                                                               |
  | 22    | SIGTTOU   | 停止直到下一个 SIGCONT | 后台进程向终端写               |                                                               |
  | 23    | SIGURC    | 忽略                   | 套接字上的紧急情况             |                                                               |
...
![Virtual Addressing](images/virtual-addressing.png)
...
![Virtual Page States](images/virtual-page-states.png)
...
![Shared Virtual Memory](images/shared-virtual-memory.png)
...
![File Descriptor Table](images/file-descriptor-table.png)

![File Sharing 1](images/file-sharing-1.png)

![File Sharing 2](images/file-sharing-2.png)

![File Sharing 3](images/file-sharing-3.png)
...
![只要有可能就使用标准 I/O，对网络套接字的 I/O 使用 RIO 函数。不要使用 scanf 或 rio_readlineb 来读取二进制文件。](images/io-functions.png)
...
![Well Known Ports](images/well-known-ports.png)
...
![@[大端序与小端序](https://www.notion.so/76162a602ba947c781dbf013e3da80ea?pvs=21) ](images/endianness.png)
...
- CGI 中定义了大量的环境变量，CGI 程序在运行时可设置这些环境变量。在 PHP 中的`$_SERVER`环境变量就是读取 CGI 的环境变量
  | 环境变量 | QUERY_STRING | SERVER_PORT    | REQUEST_METHOD | REMOTE_HOST        | REMOTE_ADDR | CONTENT_TYPE              | CONTENT_LENGTH          |
  | -------- | ------------ | -------------- | -------------- | ------------------ | ----------- | ------------------------- | ----------------------- |
  | **描述** | 程序参数     | 父进程监听端口 | GET 或 POST    | 客户端主机名或域名 | 客户端 IP   | POST 中的请求体 MIME 类型 | POST 中的请求体字节大小 |
...
![Concurrent Processes](images/concurrent-processes.png)
...
![Concurrent Threads](images/concurrent-threads.png)
