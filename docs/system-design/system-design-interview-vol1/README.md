...
- Block servers：将分块的文件上传到云存储。块存储，简称块级存储，是一种在基于云的环境中存储数据文件的技术。一个文件可以分成几个块，每个块都有一个唯一的哈希值，存储在元数据数据库中。每个块都被视为一个独立的对象并存储在对象存储(S3)中。为了重建文件，块以特定顺序连接。至于块大小，可以参考 Dropbox 的 4MB
