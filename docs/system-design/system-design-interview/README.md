...
#### 2. 每个程序员都应该知道的延迟数据 (Jeff Dean 经典指标)
...
- **文档服务器 (Document Server)**：每个处于活跃编辑状态的文档在内存中由一个服务实例维护其状态（或通过 Redis 订阅发布进行多实例同步），保证 OT 转换的序列化执行。
- **存储设计**：
    - **文档快照 (Snapshot)**：定期（如每 10 秒或 50 次操作）生成当前文档全文的快照，存储在 NoSQL（如 MongoDB）中。
    - **操作日志 (Operations Log)**：增量保存所有的编辑命令（以追加方式写入），便于回滚和历史追溯。

## 在线网盘存储 (Cloud Storage System)
...
- **客户端 (Client App)**：
    - **分块器 (Chunker)**：将大文件拆分为固定大小（如 4MB）的块（Block），以便于并行上传和断点续传。
    - **索引器 (Indexer)**：在本地维护一个轻量数据库（如 SQLite），记录文件块的哈希值与修改状态。
    - **同步管理器 (Sync Manager)**：监听本地与云端的变更，触发上传或下载。
- **元数据服务器 (Metadata Server)**：处理用户登录、文件目录结构、共享权限、块索引等元数据管理。使用具有强一致性事务的数据库（如 Sharded MySQL）。
- **文件分块服务器 (Block Server)**：负责接收客户端上传的文件块，执行**压缩**、**加密**和**去重 (Deduplication)**，然后转储到对象存储。
- **对象存储 (Object Storage)**：如 AWS S3，存储实际的加密文件块。
- **通知服务 (Notification Service)**：基于 HTTP 长轮询或 WebSocket，将云端文件更新实时推送到客户端以触发同步。

### 2. 核心技术设计

#### 1) 增量同步 (Delta Sync) 与分块传输
