...
| Password <br> ①至少有 1 位大写字母、1 位小写字母、1 位数字；<br>②允许包含!@#$%等特殊字符；<br>③密码长度至少为 8 位 | `^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[\w!@#$%^&\-]{8,}$`<br>`?=.*[a-z]`为正向肯定（Positive Lookahead）语法，`._` 表示字符串的任意位置，`._[a-z]` 表示任意位置包含小写字母                              | Password123!<br>Abc123!@                                                                                                | password<br>12345678                                  | Assertions                          |
