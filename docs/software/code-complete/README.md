...
精心计划，并不意味着事无巨细的计划或者过度的计划。你可以把房屋的结构性的支撑规划清楚，而在日后再决定是用木地板还是地毯，地面漆成什么颜色，屋顶使用什么材料，等等。
...
- <figure>
    ![子系统间缺乏通信限制](imgs/subsystem-communication-unrestricted.webp)
    <figcaption>子系统间缺乏通信限制</figcaption>
  </figure>
- <figure>
    ![施加少量通信规则后的子系统](imgs/subsystem-communication-restricted.webp)
    <figcaption>施加少量通信规则后的子系统</figcaption>
  </figure>
</div>
...
![类的冰山定律：暴露部分和隐藏细节](imgs/class-iceberg-analogy.webp){ align=right width=20% }
...
这种方法有很多优点。首先，变量名中最终的那部分，即为这一变量赋予主要含义的部分应当位于最前面，这样，这一部分就可以显得最为突出，并会被首先阅读到。其次，采纳了这一规则，你将避免由于同时在程序中使用 totalRevenue 和 revenueTotal 而产生的歧义。这些名字在语义上是等价的，上述规则可以避免将他们当做不同的东西使用。还有，类似 revenueTotal、expenseTotal、revenueAverage、expenseAverage 这组名字的变量具有非常优雅的对称性。而从 totalRevenue、expenseTotal、revenueAverage、averageExpense 这组名字中则看不出什么规律来。总之，一致性可以提高可读性，简化维护工作。
...
- 把指针操作限制在子程序或者类里面。如不要通过手动操作指针去遍历链表，应该编写一组诸如`NextLink()`、`PreviousLink()`、`InsertLink()`、`DeleteLink()`这样的访问器子程序来完成操作更好些。
...
当使用全局数据时，一个好的习惯是，将所有全局数据都冠以`_g`前缀，并且用访问器子程序来管理全局数据，除了该变量的访问器子程序以外，所有的代码都不可以访问`_g`前缀的变量。其他代码均通过访问器子程序来存取该数据。另外，全局数据在设置时，先将每个变量设置为局部变量，仅当必须时再修改为全局。犹如类中先设为`private`，再设为`public`。
...
如何解决全局数据在多线程运行时不会被意外修改呢？我们可以通过锁来控制对全局变量的访问。

**在许多情况下，全局数据事实上就是没有设计好或者没有实现好的类中的数据。在少数情况下，一些数据的确需要作为全局数据，但是可以使用访问器子程序对其进行封装，从而最大限度减少发生问题的几率。在剩余的极少数情况下，你真的需要使用全局数据。**

## 第 14 章 组织直线型代码

![直线型代码的相关语句组织方式](imgs/straight-line-code-organization.webp){ align=right width=40% }
...
NASA 的一项研究表明，通过阅读代码，每小时能发现 3.3 个 bug，而测试只能发现 1.8 个，在整个项目的生命周期中，代码 review 能比各种测试方法多发现 20%~60%的错误。
...
很多项目将测试作为软件质量的唯一组成部分，这是非常错误的做法，因为各种形式的协同开发实践都表现出比测试更高的错误检测率，而且发现一条错误的成本不到测试的一半。

你必须期望在你的代码里有错误。尽管这种期望似乎有悖常理，但是你应该期望找到这个错误的人是你，而不是别人。

如何开发出高质量的程序？
![开发高质量程序的流程](imgs/software-quality-development-process.webp){ align=right width=50% }
...
_毎个团队里都也许有这样一个程序员，他总会遇到无穷的问题：不听话的机器，奇怪的编译器错误，月圆时オ会出现的编程语言的隐藏缺陷，失效的数据，忘记做的重要改动，一个不能正常保存程序的疯狂的编辑器。_
...
- 某个类无所事事
- 一系列传递流浪数据（数据只是被传来传去而不做任何数据）的子程序
- 如果看到某个类中的绝大部分代码只是去调用其他类中的成员函数，请考虑是否应该把这样的中间人去掉，转而去直接调用其他的类
- **某个类同其他类关系过于亲密**。类的作用是使程序具备更强的可管理性，并最大限度地减少更改代码对周围的连带影响。
- 子程序命名不恰当
- 数据成员被设置为公用
...
![大型项目中沟通复杂度的增加](imgs/large-project-communication-complexity.webp){ align=right width=50% }
...
- 为评估留出时间，并作出计划
  ![布鲁克斯法则：为落后项目增加人手会使进度延后](imgs/brooks-law-adding-manpower.webp){ align=right width=40% }
...
- 代码意图说明。目的性注释用来指明一段代码的意图，它指出要解决的问题，而非解决的方法，如`//get current employee information`
...
- 不要使用，`...`等符号来对齐使注释美观，在修改这样的注释时会平添大量工作。如果花大量时间增删横线只是为了对齐注释，你就不是在编程，而是在浪费时间。去找个高效点的风格吧。有个办法是采取固定长度的横线，不管注释内容有多长。这样的横线不必维护，还能用 IDE 的宏命令方便地添加。
    ...
    //Variable      Meaning
    //--------      -------
    //xPos ........ XCoordinate Position
    //yPos ........ YCoordinate Position
...
IBM 研究指出，约每十条语句有一个注释，这样的密度使程序清晰度最高，过高或过低的 密度都会使代码难以理解。
...
知道何时放弃很难，但这是必须面对的问题。当你遭受挫折时，提出此问题正是时候。提出并不是说这时就放弃，而是该为目前的行为设置底线了：“要是这个方法 30 分钟内还不能解决问题，我就会花十分钟想些其他办法，再用一个小时尝试最可行的办法”。
