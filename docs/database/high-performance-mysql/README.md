...
- 记录锁(Record Locks)：指聚簇索引中真实存放的数据，如图中的 1、4、7、10。当我们使用唯一性的索引 (包括唯一索引和聚簇索引) 进行等值查询且精准匹配到一条记录时，此时就会直接将这条记录锁定。例如 `select * from t where id = 4 for update;` 就会将 `id=4` 的记录锁定。
    ![记录锁](imgs/innodb-record-lock.png)
...
- 临键锁(Next-Key Locks) ：指间隙加上它右边的记录组成的左开右闭区间。比图中的 (1,4]、(4,7] 等。临键锁就是记录锁 (Record Locks) 和间隙锁 (Gap Locks) 的结合，即除了锁住记录本身，还要再锁住索引之间的间隙。**当我们使用范围查询，并且命中了部分记录，此时锁住的就是临键区间**。
    ![临键锁](imgs/innodb-next-key-lock.png)
...
-  <figure>
    ![脏读示例](imgs/mysql-dirty-read.png)
    <figcaption>脏读示例</figcaption>
  </figure>
-  <figure>
    ![不可重复读示例](imgs/mysql-non-repeatable-read.png)
    <figcaption>不可重复读示例</figcaption>
  </figure>
-  <figure>
    ![幻读示例](imgs/mysql-phantom-read.png)
    <figcaption>幻读示例</figcaption>
  </figure>
-  <figure>
    ![Session B 的回滚操作使得 Session A 的写入被撤销](imgs/transaction-rollback-prevent-dirty-write.png)
...
<div class="grid cards" markdown>
-  <figure>
    ![版本链结构](imgs/innodb-mvcc-version-chain.png)
    <figcaption>版本链结构</figcaption>
  </figure>
-  <figure>
    ![MVCC 隔离级别判断过程](imgs/innodb-mvcc-read-view.png)
...
<div class="grid cards" markdown>
-  <figure>
    ![聚簇索引与非聚簇索引对比图](imgs/clustered-vs-nonclustered-index.png)
    <figcaption>聚簇索引与非聚簇索引对比图</figcaption>
  </figure>
-  <figure>
    ![聚簇索引结构与搜索过程示例](imgs/clustered-index-search-process.png)
...
<div class="grid cards" markdown>
-  <figure>
    ![一主多从](imgs/replication-one-master-multi-slaves.png)
    <figcaption>一主多从</figcaption>
  </figure>
-  <figure>
    ![主动-被动模式下的双主复制](imgs/replication-active-passive-master.png)
    <figcaption>主动-被动模式下的双主复制</figcaption>
  </figure>
-  <figure>
    ![主库-分发库-多备](imgs/replication-master-distribution-slaves.png)
