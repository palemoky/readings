...
| 可重复读 |  ✗   |     ✗      | ✗ (快照读靠 MVCC，当前读靠临键锁) | 解决不可重复读 | ① MySQL 的默认隔离级别<br>② InnoDB 和 XtraDB 通过 MVCC 解决了幻读问题                                                     |
...
**MVCC 只工作在可重复读（RR）和读已提交（RC）下**，因为 MVCC 是让不同的人看到数据不同时刻的样子，而读未提交直接没有隔离，串行化靠加锁严格隔离。
