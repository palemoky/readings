...
| 可重复读 |  ✗   |     ✗      | ✗ (加了Gap锁) | 解决不可重复读 | ① MySQL 的默认隔离级别<br>② InnoDB 和 XtraDB 通过 MVCC 解决了幻读问题                                                     |
| 串行化   |  ✗   |     ✗      |       ✗       |    解决幻读    | ①通过强制事务串行执行，避免了幻读的问题<br>② 只有在非常需要确保数据的一致性并且可以接受没有并发的情况下，才考虑采用该级别 |

> 可重复度在理论上并未解决幻读的问题，串行化才真正解决了幻读的问题。
...
非常活跃的内存交换会导致整个操作系统变得无法响应，甚至无法登陆系统。为了避免 sshd 被杀掉导致无法登陆主机的问题，可以调整 sshd 进程的 `/proc/PID/oom_adj` 或 `/proc/PID/oom_score_adj`
