...
![简单替换加密](imgs/simple-substitution-cipher.webp){ align=right width=50% }
...
一次性密码本（One-time pad, OTP）是古典密码学中的一种加密算法，利用了异或的结合律性质`a ^ b = c, a ^ c = b, b ^ c = a`，将明文与一串随机的比特序列进行异或运算。

在理论上，OTP 具有完善保密性，其安全性已被香农在 1949 年通过数学方法证明。若要保证 OTP 的安全性，需满足以下 4 个条件：
...
- HOTP（HMAC-based One-time Password algorithm，基于 HMAC 的一次性密码算法）：基于事件同步，通过某一特定的事件次序及相同的种子值作为输入，通过 HASH 算法运算出一致的密码
- TOTP（Time-based One-Time Password，基于时间的一次性密码算法）：基于时间同步，基于客户端的动态口令和验证服务器的时间比对，一般每 30 秒产生一个新口令，为了避免网络延迟与时钟不同步问题，用户设备与服务器中的时钟必须大致同步（服务器一般会接受客户端时间延迟 30 秒的时间戳生成的一次性密码）。

在生活中，TOTP 被广泛应用，如二次验证的[Google Authenticator](https://github.com/google/google-authenticator)、银行或游戏的动态令牌，其工作原理是：

- 当用户开启 2FA 时，
    1. 服务器随机生成一个类似于『DPI45HKISEXU6HG7』的密钥，并且把这个密钥保存在数据库中；
    2. 同时在页面上显示一个二维码，内容是一个 URI 地址（`otpauth://totp/ 账号？secret = 密钥`），如`otpauth://totp/username@example.com?secret=DPI45HKISEXU6HG7`；（该过程一定是基于 HTTPS 通信的）
...
![3DES 加密结构](imgs/3des-encryption.webp){ align=right width=30% }

DES 已经被破解，不再安全。3DES 就是为了增强 DES 的强度，将 DES 重复 3 次所得到的加密算法。

由于 3DES 中加入了解密操作，因此具有兼容 DES 的特性（当 3DES 中所有的密钥都相同时，就是普通的 DES，因为前两步的加密解密就是明文了）。
...
密码算法可以分为分组密码和流密码两种。
分组密码 (block cipher) 是每次只能处理特定长度的一块数据的一类密码算法，这里的“一块”就称为分组 (block)。此外，一个分组的比特数就称为分组长度 (blocklength)。
...
- <figure>
    ![RSA 的加密和解密](imgs/rsa-encryption-decryption.webp)
    <figcaption>RSA 的加密和解密</figcaption>
  </figure>
- <figure>
    ![RSA 密钥对生成](imgs/rsa-key-generation.webp)
    <figcaption>RSA 密钥对生成</figcaption>
  </figure>
</div>
...
2. 求 $L$：$L=lcm(p-1, q-1)$，$L$ 是 $p-1$ 和 $q-1$ 的最小公倍数
...
1. SWIFT（Society for Worldwide Interbank Financial Telecommunication，环球银行金融电信协会）
2. IPsec
3. SSL/TLS
...
根据 RFC 2104，HMAC 的数学公式为：$HMAC(K,m)=H((K'⊕opad) || H((K'⊕ipad) || m))$

其中：
![ipad 的 i 是 inner，opad 的 o 是 outer](imgs/hmac-padding-structure.webp){ align=right width=40% }
...
1. 序号：约定每次都对发送的消息赋予一个递增的编号 (序号)，并且在计算 MAC 值时将序号也包含在消息中。比如 TCP 协议中**初始的序号是随机值**，以避免攻击
...
在数字签名中，分别使用了非对称加密和散列函数，其中非对称加密算法的特点是：

1. **私钥加密的内容只能由公钥解密，公钥加密的内容只能由私钥解密**
2. **私钥只能由本人保管，公钥则可以随意分发**

散列函数的特点则是：

1. **任意输入都会得到一个固定长度的哈希值**
2. **输入有任何细微的变化，输入值都会有巨大的变化**

#### 原理

![数字签名与验证图解](imgs/digital-signature-verification.webp){ align=right width=40% }
数字签名工作原理如右图：
...
![HTTPS 的证书流程图](imgs/https-certificate-flow.webp){ align=right width=50% }
...
密钥协商步骤：
![Diffie-Hellman 密钥交换](imgs/diffie-hellman-key-exchange.webp){ align=right width=50% }
...
![椭圆曲线密码](imgs/elliptic-curve-cryptography.webp){ align=right width=40% }
...
!!! note "区分 ECC、DH、ECDH、ECDHE、ECDSA"

    - ECC（Elliptic Curve Cryptography）是一门利用椭圆曲线数学理论来实现加密的学科，而不是某一个具体的算法
    - DH（Diffie-Hellman）是 Diffie 和 Hellman 发明的一种**密钥交换**协议，基于离散对数，比较慢
    - ECDH（Elliptic Curve Diffie-Hellman）是基于椭圆曲线的 DH，速度快且资源占用少
    - ECDHE（Elliptic Curve Diffie-Hellman Ephemeral）是目前互联网上最主流、最安全的**密钥交换**算法，其中 E 代表 ephemeral，表示每次通信都生成新的密钥，以获得前向安全性
    - ECDSA（Elliptic Curve Digital Signature Algorithm）是基于椭圆曲线的**数字签名**算法

    在 HTTPS 的证书签名算法中，`TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256` 的含义是：

    - TLS：TLS 协议
    - ECDHE：密钥交换算法
    - ECDSA：数字签名算法
    - AES_128_GCM：对称加密算法
    - SHA256：哈希/摘要算法。用于生成伪随机数或确保数据完整性
...
**软件只能生成伪随机数列**，这是因为运行软件的计算机本身仅具备**有限的内部状态**。而在内部状态相同的条件下，软件必然只能生成相同的数，因此软件所生成的数列在某个时刻一定会出现重复。

要生成具备不可重现性的随机数列，需要从不可重现的**物理现象中获取信息**，比如周围的温度和声音的变化、用户移动的鼠标的位置信息、键盘输入的时间间隔、放射线测量仪的输出值等。
...
![基于单向散列函数的伪随机数生成器](imgs/prng-hash-function.webp){ align=right width=40% }
...
GPG（GnuPG，GNU Privacy Guard）是一款基于 OpenPGP 标准开发的密码学软件，支持加密、数字签名、密钥管理、S/MIME、SSH 等多种功能。
...
- 证书：可生成 OpenPGP 中规定格式的证书，以及与 X.509 规范兼容的证书。还可以颁发公钥的作废证明 (revocation certificate)，并可以使用 CRL 和 OCSP 对证书进行校验
...
> From [Creating and Managing a GPG Key Pair - YouTube](https://www.youtube.com/watch?v=1vVIpIvboSg)
...
> From [《Pro Git》文档](https://git-scm.com/book/zh/v2/Git-%E5%B7%A5%E5%85%B7-%E7%AD%BE%E7%BD%B2%E5%B7%A5%E4%BD%9C)

> From [使用 GPG 签署 Git 提交教程](https://www.youtube.com/watch?v=4166ExAnxmo)
...
SSL/TLS 的应用：HTTPS、SSH、SFTP、SMTP/POP3

<div class="grid cards" markdown>
- <figure>
    ![SSL/TLS 概述](imgs/ssl-tls-overview.webp)
    <figcaption>SSL/TLS 概述</figcaption>
  </figure>
- <figure>
    ![SSL/TLS 协议](imgs/ssl-tls-protocol-suite.webp)
    <figcaption>SSL/TLS 协议</figcaption>
  </figure>
- <figure>
    ![握手协议中的信息是在没有加密的情况下进行的](imgs/ssl-tls-handshake-unencrypted.webp)
    <figcaption>握手协议中的信息是在没有加密的情况下进行的</figcaption>
  </figure>
</div>

### 第 15 章 密码技术与现实社会

![密码技术与现实社会总结](imgs/cryptography-tools-summary.webp){ align=right width=40% }
