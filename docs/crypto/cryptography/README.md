...
![简单替换加密](imgs/simple-substitution-cipher.png){ align=right width=50% }
...
![3DES 加密结构](imgs/3des-encryption.png){ align=right width=30% }
...
其中：
![ipad 的 i 是 inner，opad 的 o 是 outer](imgs/hmac-padding-structure.png){ align=right width=40% }
...
![数字签名与验证图解](imgs/digital-signature-verification.png){ align=right width=40% }
数字签名工作原理如右图：
...
![HTTPS 的证书流程图](imgs/https-certificate-flow.png){ align=right width=50% }
...
密钥协商步骤：
![Diffie-Hellman 密钥交换](imgs/diffie-hellman-key-exchange.png){ align=right width=50% }
...
![椭圆曲线密码](imgs/elliptic-curve-cryptography.png){ align=right width=40% }
...
![基于单向散列函数的伪随机数生成器](imgs/prng-hash-function.png){ align=right width=40% }
...
![密码技术与现实社会总结](imgs/cryptography-tools-summary.png){ align=right width=40% }
